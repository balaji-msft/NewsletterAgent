"""
Fabric Platform Newsletter Agent.
...
from fabricplatform import tools
from shared.openai_client import create_openai_client

logger = logging.getLogger("fabricplatform.agent")
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                ...
                "component (team name), page, and wiki_link."
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    "folder_filter": {
                        "type": "string",
                        "description": (
                            "Wiki folder path to scope to, e.g. '/Fabric Platform'. "
                            "Defaults to TSG_WIKI_FOLDER config value if omitted."
                        ),
                    },
                    "days_back": {
                        "type": "integer",
                        "description": "Number of days to look back. Ignored when year+month are provided.",
                    },
                    "year": {
                        "type": "integer",
                        "description": "Year to scope commits to (e.g. 2026). Use with month for exact month range.",
                    },
                    "month": {
                        "type": "integer",
                        "description": "Month number (1-12) to scope commits to. Use with year for exact month range.",
                    },
                },
                "required": [],
            },
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                ...
                "whose title contains a substring (default: NF-PLAT). "
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "title_filter": {
                        "type": "string",
                        "description": "Substring to filter page titles (default: NF-PLAT).",
                    },
                },
                "required": [],
            },
...
def _create_log_dir() -> Path:
    ...
    log_dir = Path(__file__).resolve().parent.parent / "logs" / f"fabricplatform_{ts}"
...
def run_fabricplatform_newsletter_agent(user_prompt: str | None = None) -> str:
    """
    Run the Fabric Platform Newsletter Agent.
    ...
    logger.info("Fabric Platform Newsletter tool output logs: %s", log_dir)
    ...
    for round_num in range(MAX_TOOL_ROUNDS):
        ...
        logger.info("Fabric Platform Newsletter agent finished after %d rounds.", round_num + 1)
        if captured_html:
            logger.info("Returning captured HTML from send_email (%d chars).", len(captured_html))
            return captured_html
        return final_text

    return "Fabric Platform Newsletter agent hit maximum tool-call rounds without producing a final response."
