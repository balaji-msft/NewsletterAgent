...
from dnai import tools
from shared.openai_client import create_openai_client

logger = logging.getLogger("dnai.agent")
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                ...
                "whose title contains a configured substring. "
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "title_filter": {
                        "type": "string",
                        "description": "Substring to filter page titles.",
                    },
                },
                "required": [],
            },
...
def _create_log_dir() -> Path:
    ...
    log_dir = Path(__file__).resolve().parent.parent / "logs" / f"dnai_{ts}"
...
def run_dnai_newsletter_agent(user_prompt: str | None = None) -> str:
    """
    Run the DnAI Newsletter Agent.

    Compiles a monthly HTML newsletter.
    """
    ...
    logger.info("DnAI Newsletter tool output logs: %s", log_dir)
    ...
    for round_num in range(MAX_TOOL_ROUNDS):
        ...
        logger.info("DnAI Newsletter agent finished after %d rounds.", round_num + 1)
        if captured_html:
