"""
Fabric BI MoR (Monthly Operational Review) Callout Agent.
...
TOOL_DEFINITIONS = [
    {
        "type": "function",
        "function": {
            "name": "get_mor_query_results",
            "description": (
                "Execute the saved Azure DevOps work-item query for MoR data. "
    ...
    },
    {
        ...
        "function": {
            "name": "send_email",
            "description": (
                "Send the compiled MoR callouts HTML via email using Power Automate webhook."
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    ...
                    "html_body": {
                        "type": "string",
                        "description": "Full HTML body of the MoR callouts email.",
                    },
                    "to_recipients": {
                        "type": "string",
...
def run_mor_agent(user_prompt: str | None = None) -> str:
    """
    Run the MoR Callout Agent.

    1. Fetches ADO query data via function calling.
    2. Produces HTML callouts for MoR.
    ...
    return "MoR agent hit maximum tool-call rounds without producing a final response."
