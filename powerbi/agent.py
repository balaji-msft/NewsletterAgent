...
from powerbi import config
from powerbi import tools
from shared.openai_client import create_openai_client

logger = logging.getLogger("powerbi.agent")
...
def _create_log_dir() -> Path:
    ...
    log_dir = Path(__file__).resolve().parent.parent / "logs" / f"powerbi_{ts}"
...
def run_powerbi_newsletter_agent(user_prompt: str | None = None) -> str:
    """
    Run the Power BI Newsletter Agent.
    ...
    logger.info("Power BI Newsletter tool output logs: %s", log_dir)
    ...
    for round_num in range(MAX_TOOL_ROUNDS):
        ...
        logger.info("Power BI Newsletter agent finished after %d rounds.", round_num + 1)
        if captured_html:
            logger.info("Returning captured HTML from send_email (%d chars).", len(captured_html))
            return captured_html
        return final_text

    return "Power BI Newsletter agent hit maximum tool-call rounds without producing a final response."
